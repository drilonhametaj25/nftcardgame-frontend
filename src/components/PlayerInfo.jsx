...
const healtLevel = (points) => (points >= 12 ? 'bg-green-500' : 'points' >= 6 ? 'bg-orange-500': 'bg-red-500')
        ...
        <div className={`${styles.flexCenter} ${mt ? 'mt-4': 'mb-4'}`}>
            ...
            >
                {[...Array(player.health).keys()].map((item,index) => (
                    <div key={`player-item-${item}`} className={`${styles.playerHealthBar} ${healtLevel(player.health)} ${marginIndexing(index)}`}>
                    </div>
                ))}
            ...
            <ReactTooltip id={`Healt-${mt ? '1' : '2'}`} effect="solid" backgroundColor="#7f46f0"/>
